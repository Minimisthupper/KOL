...
"""
    ...
    - die dynamischen Paramter alpha, beta, gamma und rho sind im internen Array 'parameter_' abgelegt
    Funktions-Argumente:
    - initiale_werte:
...
class SEIR:
    """
        ...
        - wird ausgefuehrt um die Klasse zu initialisieren (ein Objekt zu erstellen)
        - dient der Zuweisung von Werten zu Objekt-Attributen
        ...
        - Quelle fuer reale Covid-Werte aus dem Jahre 2019:
        https://www.thelancet.com/journals/langlo/article/PIIS2214-109X(20)30074-7/fulltext
    """
    def __init__(
        self,
        # Anfaellige, Ausgesetzte, Infizierte, Genesene
    ...
    def setze_parameter(
        ...
        if verbose:
            ...
            print("alpha (Inkubationszeit):   ",self.alpha)
            print("beta  (Uebertragunsrate):  ",self.beta)
            print("gamma (Erholungszeit):     ",self.gamma)
            print("rho   (Social Distancing): ",self.rho)

    """
        - zuruecksetzen der internen Liste in den Ausgangsstauts ('zero-state')
    """
...
#### ab hier: Zeichenfunktionen ####
    ...
    def zeichnen(
        ...
        class matplotlib.figure.Figure(
            ...
            constrained_layout=None, *, layout=None,
            **kwargs)
        """
        ...
        # Legende
        plt.legend(
            ['Anfaellige (Susceptible)', 'Ausgesetzte (Exposed)', 'Infizierte (Infected)', 'Genesene (Recovered)'],
            fontsize=20)

        # x-Achse
        plt.xlabel(
            'Zeitintervall (%s Tage)' % tage,
            fontsize=20)

        # y-Achse
        plt.ylabel(
            'Anteil der Bevoelkerung (in %)',
            fontsize=20)
    ...
    def zeichnen_variable(
        self,
        var,
        var_name=None,
        show=True):
        """
        Zeichnen der gegebenen Variable
        Erwartet eine Liste oder Numpy-Array als Variable
        """
        if var is None:
            var = self.i
